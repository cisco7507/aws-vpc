...
variable "create_private_subnet_route_table" {
  description = "Controls if separate route table for private subnets should be created"
  type        = bool
  default     = true
}

variable "create_private_internet_gateway_route" {
  description = "Controls if an internet gateway route for Internet private subnets should be created"
  type        = bool
  default     = true
}
...
variable "one_nat_gateway_per_az" {
  description = "Should be true if you want only one NAT Gateway per availability zone. Requires `var.azs` to be set, and the number of `firewall_subnets` created to be greater than or equal to the number of availability zones specified in `var.azs`."
  type        = bool
  default     = false
}
...
variable "external_nat_ips" {
  description = "List of EIPs to be used for `nat_firewall_ips` output (used in combination with reuse_nat_ips and external_nat_ip_ids)"
  type        = list(string)
  default     = []
}
...
variable "propagate_firewall_route_tables_vgw" {
  description = "Should be true if you want route table propagation"
  type        = bool
  default     = false
}
...
variable "ingress_route_table_tags" {
  description = "Additional tags for the protected route tables"
  type        = map(string)
  default     = {}
}
...
variable "create_igw" {
  description = "Controls if an Internet Gateway is created for firewall subnets and the related routes that connect them."
  type        = bool
  default     = true
}
