...
| <a name="input_private_subnets_mapping"></a> [private\_subnets\_mapping](#input\_private\_subnets\_mapping) | Ordered list of subnet names. The order should be set to match the output of the private\_subnets variable, as established by its CIDR function, i.e the first CIDR block returned by the function will cause the subnet to be named as per the first value of the private\_subnets\_mapping (this variable) and so on. | `list(string)` | `[]` | no |
